...
// START OF NEW IMPORTS for TensorFlow Lite
...
public class DeepScanProcessor {
    ...
    /**
     * Runs the style prediction model on a given style bitmap.
     *
     * @param styleBitmap The bitmap of the artistic style (e.g., charcoal sketch).
     * @param predictionInterpreter The initialized TFLite interpreter for magenta_prediction.tflite.
     * @return A 1D float array representing the calculated style vector.
     */
    public static float[] runStylePrediction(Bitmap styleBitmap, Interpreter predictionInterpreter) {
        ...
        tensorStyleImage.load(styleBitmap);
        tensorStyleImage = imageProcessor.process(tensorStyleImage);

        // Define the output buffer for the style vector
        // The shape is [1, 1, 1, 100] as specified by the model
    ...
    /**
     * Applies the calculated style vector to a content bitmap to generate the stylized image.
     *
     * @param contentBitmap The video frame to be stylized.
     * @param styleVector The float array generated by the style prediction model.
     * @param transferInterpreter The initialized TFLite interpreter for magenta_transfer.tflite.
     * @return A new bitmap containing the stylized version of the content image.
     */
    public static Bitmap runStyleTransfer(Bitmap contentBitmap, float[] styleVector, Interpreter transferInterpreter) {
        ...
        // Post-process the output buffer back into a Bitmap
        ...
        Bitmap stylizedBitmap = Bitmap.createBitmap(pixels, 384, 384, Bitmap.Config.ARGB_8888);
