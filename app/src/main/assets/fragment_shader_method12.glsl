...
void main() {
    ...
    // --- FIX START: Restore the correct if/else logic to separate person and background ---

    // Check if the current pixel is part of the PERSON (where the mask is white)
    if (mask.r > 0.5) {
        // --- Apply Pencil Sketch Logic ---
        ...
        // The final color for this pixel is the SKETCH.
        gl_FragColor = vec4(vec3(finalGray), 1.0);

    } else {
        // --- This is the BACKGROUND, so the final color is solid WHITE. ---
        gl_FragColor = vec4(1.0, 1.0, 1.0, 1.0);
    }
    
    // --- FIX END ---
    // The separate edge detection/outline code is now permanently removed.
}
